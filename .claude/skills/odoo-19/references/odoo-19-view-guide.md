...
<search string="Search My Models">
    ...
    <!-- Advanced filter (name is required on every filter) -->
    <filter string="My Partners" name="my_partners" domain="[('partner_id.user_id', '=', uid)]"/>

    <!-- Separator -->
    <separator/>

    <!-- Group by: the group element takes no attributes in a search view.
         expand= and string= are rejected by search_view.rng in Odoo 19. -->
    <group>
        <filter string="State" name="state" context="{'group_by': 'state'}"/>
